    ...
    <div v-if="cart.items.length > 0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-4">
        <div
          ...
          :key="item.product.id"
          class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col sm:flex-row gap-4 sm:items-center"
        >
          <div class="flex gap-4 flex-1 min-w-0">
            <RouterLink :to="`/product/${item.product.id}`" class="flex-shrink-0">
              <img :src="item.product.images[0]" :alt="item.product.name" class="w-20 h-20 object-cover rounded-xl" />
            </RouterLink>
            <div class="flex-1 min-w-0">
              <RouterLink :to="`/product/${item.product.id}`">
                <p class="font-semibold text-slate-900 hover:text-brand-700 transition-colors truncate">{{ item.product.name }}</p>
              </RouterLink>
              <p class="text-xs text-slate-400 font-mono mt-0.5 truncate">{{ item.product.sku }}</p>
          ...
          <div class="flex items-center justify-between gap-3 flex-shrink-0">
            <div class="flex items-center border border-slate-200 rounded-xl overflow-hidden bg-slate-50 flex-shrink-0">
            ...
            <p class="font-extrabold text-slate-900 flex-shrink-0 whitespace-nowrap">{{ formatCurrency(cart.lineUnitPrice(item) * item.quantity) }}</p>
            <button @click="cart.removeFromCart(item.product.id)" :aria-label="`Eliminar ${item.product.name} del carrito`" class="text-slate-300 hover:text-red-400 transition-colors flex-shrink-0">
